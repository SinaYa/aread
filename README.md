# ai-assistant-reader (`aread`)

An AI assistant for AI assistants.
...
- **Read once, answer many.** Pass several `--task` flags and the file is
...
Edit [`config.toml`](config.toml) at the project root:
...
The config file is found by absolute path (next to this README), so `aread`
resolves it the same no matter which directory you run the global command from.
